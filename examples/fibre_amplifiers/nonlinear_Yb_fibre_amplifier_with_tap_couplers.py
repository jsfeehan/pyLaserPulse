...
# -*- coding: utf-8 -*-
"""
The following is an example of a basic nonlinear Yb-fibre amplifier with a 5%
tap coupler at the input.

The tapped light is stored in pulse.output, which is a list whose length is
equal to the number of output couplers in an amplifier chain. Each list element
is a numpy array of the field at that point, multiplied by the tap fraction.

The tapped light is not plotting by the optical_assemblies module, but it is
saved if directory != None.
"""
...
################################################################
component_list = [tap, iso_wdm, ydf]
